...
function buildAdaptRecipeInstructions(
  ...
  return [
    ...
    "FILL-LEVEL PHYSICS: The 3-liter jug is tall and tapered, with a smaller base than rim. Do not translate baking-tray or frying-pan liquid/fat amounts literally. Before every cooking step, estimate whether ingredients are meant to be merely coated, partly surrounded, or immersed. For confit, poaching, boiling, braising, or cooking dry absorbent foods, add enough water, stock, or oil to surround the food at the required level while staying under the 3-liter maximum. A pan amount such as 2-4 tbsp oil is only enough for sautéing/aromatics, not for confit or immersion in the jug.",
    ...
    "PRACTICAL WEIGHING AND MARKET QUANTITIES: The MC3 scale is only accurate to about 10-20 g in real use, so use scale steps only for larger quantities where that tolerance is acceptable. Do not use scale steps for spices, herbs, leavening, or other very small amounts; express those as teaspoons, tablespoons, pinches, pieces, or other normal recipe measures instead. For easily scalable main ingredients, especially proteins, choose quantities that are normal to buy in the target country and locale rather than arbitrary gram values. For Germany, for example, prefer common package sizes such as 400 g or 500 g ground beef when the jug capacity and recipe scaling allow it; avoid unusual amounts like 360 g or 750 g unless the source, texture, or capacity constraints make that quantity necessary.",
    ...
    "HEAT-UP TIME BUDGET: Source recipes often give pan/stovetop cooking times after preheating, but Monsieur Cuisine mode durations may start with a cold or partly cold jug. Add heat-up time only when the step itself must bring cold contents up to cooking temperature; do not add a large blanket offset when the previous step already heated the liquid or food. For one-pot pasta, follow the official MC pattern: heat the liquid first, add pasta through the lid opening, use the package cooking time plus about 3 minutes, then add a check step for doneness and an optional short extra-cooking step. Do not apply a large extra heat-up offset after the liquid is already hot. For roast/manualCooking from a cold jug, add only a modest heat-up allowance, usually 2-5 min for small aromatics and 5-8 min for larger vegetable/meat loads. Do not copy frying-pan or boiling times directly, but avoid overcooking already-hot contents.",
...
3. Scale step description: Must instruct the user to prepare the ingredient and specify the net weight to weigh. Format this exact instruction using the localized template: "${localeGuidance.accessoryPhrases.grossNetWeightTemplate}" by substituting {ingredient} with the name of the ingredient and {net} with the net weight. Ensure the entire step description (including this instruction) is at or below 240 characters.`,
    ...
    `SIMMERING BASKET LIQUID LEVEL: Decide whether the basket contents should be steamed above liquid or boiled/poached while immersed. 500 ml/g liquid is only the minimum for steam generation. For potatoes, lentils, pasta, eggs, or any basket step that is meant to boil rather than steam, you MUST add enough liquid to reach and surround the food in the basket, usually about 1200-1800 g depending on food volume, without exceeding the 3-liter jug limit. In the manual step before basket cooking, state the exact liquid amount and include this localized phrase when the minimum applies: "${localeGuidance.accessoryPhrases.minLiquidSimmeringBasket}". The entire preceding step description must be at or below 240 characters.`,
    ...
    "LIQUID ADJUSTMENT: Because the device is a sealed environment with low evaporation, reduce free simmering liquids by about 10-20% only when the goal is a sauce, soup, stew, or braise that is not meant to immerse ingredients. Do NOT reduce liquid or oil below the level needed for boiling in the basket, poaching, confit, cooking dry absorbent foods, or preventing scorching.",
...
export function accessoryHardwareRules(
  ...
  return [
    ...
    `- ${terms.simmeringBasket}: Use for boiling sides (like potatoes, rice, pasta) inside the jug. PHYSICAL LIMIT: The basket is much smaller than the jug. It can hold a maximum of approx. 1000 g to 1200 g of solid food. If the recipe requires more, it must be scaled down. The step must explicitly instruct the user to lift/remove the basket using the spatula's hook: "${phrases.liftSimmeringBasket}". For roasting, searing, or high-temperature reduction steps, instruct the user to remove the ${terms.measuringCup} and place the simmering basket on top of the lid as a splash guard: "${phrases.useBasketAsSplashGuard}" to allow steam to escape while preventing splattering. CRITICAL LIQUID LEVEL: 500 ml/g is only enough to create steam. If the food in the basket is meant to boil or simmer immersed (potatoes, lentils, pasta, eggs), add enough liquid to reach and surround the food in the basket, usually about 1200-1800 g depending on food volume, while keeping the total jug contents below 3 liters. State the exact liquid amount in the preceding manual step; when using the steam minimum, include: "${phrases.minLiquidSimmeringBasket}".`,
